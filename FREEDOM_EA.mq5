...
double GetBidLikeMQL4()
{
   MqlTick tick;
   if(SymbolInfoTick(_Symbol, tick))
      return NormalizeDouble(tick.bid, _Digits);
   return NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits);
}

double GetAskLikeMQL4()
{
   MqlTick tick;
   if(SymbolInfoTick(_Symbol, tick))
      return NormalizeDouble(tick.ask, _Digits);
...
void OnTick()
{
   ...
   double currentPrice = GetBidLikeMQL4();
...
{
   double atrSL = GetATR(PERIOD_M1, 4, 1);
   double above_Candle_SL = GetAskLikeMQL4() + (ATR_SL * atrSL);
   double check = GetAskLikeMQL4() + (Max_SL * _Point);
...
{
   double atrSL = GetATR(PERIOD_M1, 4, 1);
   double below_Candle_SL = GetBidLikeMQL4() - (ATR_SL * atrSL);
   double check = GetBidLikeMQL4() - (Max_SL * _Point);
...
void modif_Buy()
{
   ...
   if(!ConstSL)
   {
      if((!HalfTP_SL && stopLoss < openPrice) || Infinite_SL)
         stopLoss = NormalizeDouble(GetBidLikeMQL4() - atrSL * ATR_SL, _Digits);
...
void modif_Sell()
{
   ...
   {
      if((!HalfTP_SL && stopLoss > openPrice) || Infinite_SL)
         stopLoss = NormalizeDouble(GetAskLikeMQL4() + atrSL * ATR_SL, _Digits);
